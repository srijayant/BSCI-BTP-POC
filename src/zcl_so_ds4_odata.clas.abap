CLASS zcl_so_ds4_odata DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_SO_DS4_ODATA IMPLEMENTATION.


  METHOD if_rap_query_provider~select.

    DATA: ls_entity_key     TYPE zzodata_zc_rap_sales619655f80d,
          lt_business_data  TYPE TABLE OF zzodata_zc_rap_sales619655f80d,
          ls_business_data  TYPE zzodata_zc_rap_sales619655f80d,
          lo_http_client    TYPE REF TO if_web_http_client,
          lo_client_proxy   TYPE REF TO /iwbep/if_cp_client_proxy,
          lo_request        TYPE REF TO /iwbep/if_cp_request_read_list,
          lo_response       TYPE REF TO /iwbep/if_cp_response_read_lst,
          lt_so             TYPE TABLE OF zce_so_ds4,
          ls_so             TYPE zce_so_ds4,
...
************************************************************************************get data from ds4
              TRY.
                  ...
                  lo_request = lo_client_proxy->create_resource_for_entity_set('ZC_RAP_SALESDOC' )->create_request_for_read( ).
...
************************************************************************************custom entity implementation
              ...
              WHERE sales_order = @lt_business_data-Vbeln
              INTO TABLE @DATA(lt_glb_wo).

              IF lv_details_read = abap_false.
                LOOP AT lt_business_data INTO ls_business_data.
                  ls_so = CORRESPONDING #( ls_business_data ).
                  DATA(ls_glb_wo) = VALUE #( lt_glb_wo[ sales_order = ls_business_data-Vbeln ] DEFAULT space ).
                  ls_so-WorkOrder = ls_glb_wo-work_order.
                  APPEND ls_so TO lt_so.
                  CLEAR ls_so.
                ENDLOOP.
              ELSE.
                READ TABLE lt_filter_cond WITH KEY name = 'VBELN' INTO DATA(ls_productid_filter_key).
                IF sy-subrc = 0 AND lines( ls_productid_filter_key-range ) = 1.
                  ...
                  ls_so = CORRESPONDING #( ls_business_data ).
                  ls_glb_wo = VALUE #( lt_glb_wo[ sales_order = ls_business_data-Vbeln ] DEFAULT space ).
                  ls_so-WorkOrder = ls_glb_wo-work_order.
                  ls_so-gwo_uuid = ls_glb_wo-gwo_uuid.
                  ls_so-created_by = ls_glb_wo-created_by.
                  ls_so-created_on = ls_glb_wo-created_on.
                  APPEND ls_so TO lt_so.
                  CLEAR ls_so.
                ENDIF.
              ENDIF.
  ...
  METHOD is_key_filter.
    ...
    READ TABLE it_filter_cond WITH KEY name = 'VBELN' INTO DATA(ls_productid_filter_key).
    IF sy-subrc = 0 AND lines( ls_productid_filter_key-range ) = 1.
      READ TABLE ls_productid_filter_key-range INTO DATA(ls_id_option) INDEX 1.
      IF sy-subrc = 0 AND ls_id_option-sign = 'I' AND ls_id_option-option = 'EQ' AND ls_id_option-low IS NOT INITIAL.
